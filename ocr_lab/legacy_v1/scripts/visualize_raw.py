...
def main():
    ...
    # Changed input/output filenames
    json_path = os.path.join(target_dir, "ocr_output_roi_raw.json")
    output_image_path = os.path.join(target_dir, "ocr_reconstructed_raw.png")
    
    # Canvas settings (Screen size assumed)
    SCREEN_W = 1920
    SCREEN_H = 1080
    ...
    print(f"Found {len(items)} raw items to visualize.")
    ...
    for item in items:
        ...
        # Draw box (Blue for RAW to distinguish)
        draw.rectangle([x1, y1, x2, y2], outline=(255, 255, 0), width=1) # Cyan/Yellowish
    ...
    print(f"Saved RAW visualization to: {output_image_path}")
